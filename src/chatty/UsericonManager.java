...
public class UsericonManager {
    ...
    private synchronized void addFallbackIcons() {
        addFallbackIcon(Usericon.TYPE_MOD, "icon_mod.png");
        addFallbackIcon(Usericon.TYPE_ADMIN, "icon_admin.png");
        addFallbackIcon(Usericon.TYPE_STAFF, "icon_staff.png");
        addFallbackIcon(Usericon.TYPE_BROADCASTER, "icon_broadcaster.png");
        addFallbackIcon(Usericon.TYPE_SUB, "icon_sub.png");
        addFallbackIcon(Usericon.TYPE_TURBO, "icon_turbo.png");
    }
    
    private void addFallbackIcon(int type, String fileName) {
     ...
     */
    public synchronized ImageIcon getIcon(int type, User user) {
     ...
     */
    private ImageIcon getDefaultIcon(int type, User user, int source) {
     ...
     */
    public synchronized List<ImageIcon> getCustomIcons(int type, User user) {
        List<ImageIcon> result = new ArrayList<>();
        if (customUsericonsEnabled()) {
            for (Usericon icon : customIcons) {
                if (icon.type == type && iconMatchesUser(icon, user) && icon.image != null) {
    ...
    private boolean iconMatchesUser(Usericon icon, User user) {
        ...
        if (icon.restriction == null) {
            return true;
        } else if (icon.matchType == Usericon.MATCHTYPE_ALL) {
            return true;
        } else if (icon.matchType == Usericon.MATCHTYPE_NAME) {
            if (icon.restrictionValue.equalsIgnoreCase(user.getNick())) {
                return true;
            }
        } else if (icon.matchType == Usericon.MATCHTYPE_CATEGORY) {
            if (user.hasCategory(icon.category)) {
                return true;
            }
        } else if (icon.matchType == Usericon.MATCHTYPE_STATUS) {
