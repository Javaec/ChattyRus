...
public class CustomCommands {
     ...
     */
    public synchronized String command(String command, String parameter) {
        ...
        if (commands.containsKey(command)) {
            ...
            return new ParameterReplacer().replace(commandText, parameters);
        }
        return null;
    }
    ...
    public synchronized boolean containsCommand(String command) {
        return commands.containsKey(Helper.toLowerCase(command));
    }
        ...
        for (String c : commandsToLoad) {
            if (c != null && !c.isEmpty()) {
                ...
                if (split.length == 2) {
                    ...
                    if (!command.isEmpty()) {
                        commands.put(Helper.toLowerCase(command), split[1]);
                    }
                }
            }
        }
