        ...
        try (BufferedReader reader = Files.newBufferedReader(FILE, CHARSET)) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (!line.isEmpty()) {
                    ...
                    names.put(Helper.toLowerCase(name), new CapitalizedName(name, time, fixed));
                }
            }
    ...
    public String commandRefreshCase(String parameter) {
        if (parameter == null) {
            ...
            if (!Helper.validateStream(name)) {
                ...
                synchronized(this) {
                    CapitalizedName cached = names.get(Helper.toLowerCase(name));
                    if (cached != null && cached.fixed) {
                        names.remove(Helper.toLowerCase(name));
                        name = cached.capitalizedName;
                        fixed = true;
                    }
                }
                api.requestUserInfo(Helper.toLowerCase(name));
                if (fixed) {
    ...
    public synchronized String commandSetCase(String parameter) {
        if (parameter == null) {
            ...
            if (!Helper.validateStream(name)) {
                ...
                setName(Helper.toLowerCase(name), name, true);
